...
exports.execute = async (client, message, args = []) => {
  ...
  const gridWidth = 7; // sütun sayısı
  const gridHeight = 10; // satır sayısı (yukarıdan aşağıya)
  ...
  collector.on("collect", async (int) => {
    try {
      ...
      await int.update({
        content: `✅ ${chosen.label} başlatılıyor...`,
        embeds: [],
        components: [],
      });
  ...
  async function startGame(client, triggerMessage, diff) {
    const cfg = {
      ...
      trainIntensity: diff.trainCount, // multiplier for train frequency/number
      minEditInterval: 100,
      hasLeaderboard: true,
    };

    let playerX = Math.floor(cfg.gridWidth / 2);
    let playerY = cfg.gridHeight - 1; // player row (can move up/down)
    const maxUp = 0; // top row index
    const minDown = cfg.gridHeight - 1;

    let obstacles = []; // vertical-moving cars {x,y}
    let trains = []; // horizontal trains {row, x, dir, length}
    let apple = null; // {x,y}

    let speed = cfg.baseSpeed;
    let tick = 0;
    let score = 0; // her geçen satır için +1, geçiren araba için +5
    let dodged = 0;
    let gameActive = true;
    ...
    // ultimate
    let ultCharges = 1; // maksimum şarj sayısı
    const ultMax = 1;
    let ultActiveTicks = 0; // atlama sırasında çarpışma yok
    const ultDurationTicks = 3;
    let dodgedSinceUlt = 0; // doldurma için
    ...
    function spawnObstacle() {
      // spawn 0..2 obstacles depending on difficulty and time
      const count = Math.random() < 0.6 ? 1 : 2;
      for (let i = 0; i < count; i++) {
        const x = Math.floor(Math.random() * cfg.gridWidth);
        // avoid spawning directly under player if player is bottom? it's fine
        obstacles.push({ x, y: 0 });
      }
    }

    function spawnTrain() {
      // pick a row between 1 and playerY-1 (can't spawn on player's current row immediately)
      const possibleRows = [];
      ...
      // starting x outside grid so it moves in
      const x = dir > 0 ? -length : cfg.gridWidth + length - 1;
      trains.push({ row, x, dir, length });
    }

    function placeApple() {
      // 25% chance to spawn if none exists
      if (apple) return;
      if (Math.random() < 0.25) {
    ...
    function moveObstacles() {
      ...
      for (const o of obstacles) {
        if (o.y > playerY) {
          ...
          dodged++;
          dodgedSinceUlt++;
          // recharge ult when dodged enough
          if (dodgedSinceUlt >= 5) {
            dodgedSinceUlt = 0;
    ...
    function moveTrains() {
      const rem = [];
      for (const t of trains) {
        t.x += t.dir; // move horizontally
        // if completely out of bounds, remove
    ...
    function baseEmbed() {
      return new MessageEmbed()
        .setTitle(`🚗 Araba Kaçış • ${diff.label.toUpperCase()}`)
        .setDescription(
          `Puan: **${score}** • Geçirilen: **${dodged}** • Ult: ${ultCharges}/${ultMax} • UltAktif: ${
            ultActiveTicks > 0 ? ultActiveTicks : 0
          }\n\n${renderGrid()}`
        )
        .setColor("#00AA00")
        .setFooter({
    ...
    const compFilter = (i) =>
      i.user.id === playerId && i.message.id === gameMessage.id;
    ...
    function checkCollision() {
      if (ultActiveTicks > 0) return false; // invulnerable during ult
      // obstacles
      if (obstacles.some((o) => o.x === playerX && o.y === playerY))
        return true;
      // trains
      for (const t of trains) {
        for (let i = 0; i < t.length; i++) {
    ...
    async function endGame(reason) {
      ...
      const endEmbed = new MessageEmbed()
        ...
        .setColor("#FF0000")
        .setFooter({
          text: `Geçirilen: ${dodged} • Süre: ${Math.round(
    ...
    async function scheduleTick() {
      ...
      // spawn obstacles (vertical cars)
      if (
        tick % Math.max(1, Math.round(cfg.spawnEvery / (speed || 1) / 100)) ===
        0
      ) {
        // scale spawn count with time
        if (Math.random() < Math.min(0.8, 0.2 + tick / 2000)) spawnObstacle();
      }

      // spawn trains sometimes (based on time + difficulty)
      if (
        Date.now() - lastTrainSpawn >
        Math.max(1000, cfg.trainEvery / Math.max(1, 1 + tick / 800))
      ) {
        // number of trains increases with difficulty and time
        const num =
          Math.random() < 0.5
            ? 1
            : Math.ceil(cfg.trainIntensity * (1 + tick / 2000));
        for (let i = 0; i < num; i++) spawnTrain();
        lastTrainSpawn = Date.now();
      }

      // occasionally spawn apple on top row
      if (Math.random() < 0.06) placeApple();

      // move obstacles and trains
      moveObstacles();
      moveTrains();

      // collect apple if on same cell
      if (apple && apple.x === playerX && apple.y === playerY) {
        score += 100;
        apple = null;
        // grant ult charge on apple
        if (ultCharges < ultMax) ultCharges++;
      }

      // every tick add score for survival
      score += 1;
      ...
      // speed up slowly every 12 seconds
