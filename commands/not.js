exports.execute = async (client, message, args) => {
  ...
  if (!subcommand) {
    return message.reply(
      "⚠️ **Lütfen bir alt komut belirtin: ekle, sil, düzenle, veya göster.**"
    );
  }
  ...
  // **Düzenleme** Komutu (3 alt komuta ayrılmıştır)
    ...
    if (!editAction) {
      return message.reply(
        "⚠️ **Lütfen düzenleme işlemi için bir alt komut belirtin: ekle, sil, veya değiştir.**"
      );
    }
    ...
    if (editAction.toLowerCase() === "ekle") {
      ...
      try {
        ...
        await client.db.set(`note_${message.author.id}`, note);
        return message.reply(
          `✅ **Notunuza başarıyla eklendi!**\nYeni Not: ${note}`
        );
      } catch (error) {
        console.error("Not düzenlenirken hata oluştu:", error);
        return message.reply(
          "❌ **Not düzenlenirken bir hata oluştu. Lütfen tekrar deneyin.**"
        );
      }
    } else if (editAction.toLowerCase() === "sil") {
      // Kullanım: not düzenle sil <kelime|harf> <değer>
      const silTip = args[2];
      const target = args.slice(3).join(" ");
      if (!silTip || !target) {
        return message.reply(
          "⚠️ **Lütfen 'sil' için tür (kelime veya harf) ve silinecek değeri girin!**"
        );
      }
      try {
        if (silTip.toLowerCase() === "kelime") {
          ...
          return message.reply(
            "⚠️ **Lütfen 'sil' için geçerli bir tür belirtin: kelime veya harf.**"
          );
        }
        await client.db.set(`note_${message.author.id}`, note);
        return message.reply(
          `✅ **Notunuzdan başarıyla silindi!**\nYeni Not: ${note}`
        );
      } catch (error) {
        console.error("Not düzenlenirken hata oluştu:", error);
        return message.reply(
      ...
      // Kullanım: not düzenle değiştir <kelime|harf> <index> <yeni değer>
      const degistirTip = args[2];
      const index = args[3];
      const newValue = args.slice(4).join(" ");
      if (!degistirTip || index === undefined || !newValue) {
        return message.reply(
          "⚠️ **Lütfen 'değiştir' için tür (kelime veya harf), index ve yeni değeri girin!**"
        );
      }
      try {
        if (degistirTip.toLowerCase() === "kelime") {
          ...
          return message.reply(
            "⚠️ **Lütfen 'değiştir' için geçerli bir tür belirtin: kelime veya harf.**"
          );
        }
        await client.db.set(`note_${message.author.id}`, note);
        return message.reply(
      ...
      return message.reply(
        "⚠️ **Geçersiz düzenleme alt komutu! Lütfen 'ekle', 'sil' veya 'değiştir' kullanın.**"
      );
    }
  }
  ...
  // Geçersiz alt komut
    ...
    return message.reply(
      "⚠️ **Geçersiz alt komut! Lütfen 'ekle', 'sil', 'düzenle' veya 'göster' kullanın.**"
    );
  }
};
