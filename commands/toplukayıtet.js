...
exports.execute = async (client, message, args) => {
  try {
    ...
    if (missing.length > 0) {
      return message.reply(
        `❌ Bu komutu çalıştırabilmem için bana şu izinler verilmiş olmalı: ${missing
          .map((p) => `\`${p}\``)
          .join(", ")}`
      );
    }

    // Alt komut ve rol kontrollerine geç
    ...
    if (!uyeRol || !unregisteredRol) {
      return message.reply(
        "❌ Lütfen `@üye` ve `@unregistered` rollerini etiketleyin."
      );
    }
    ...
    if (unregisteredMembers.size === 0) {
      return message.reply("⚠️ Kayıtsız kullanıcı bulunamadı.");
    }
    ...
    for (const member of unregisteredMembers.values()) {
      try {
        ...
        if (sub === "cinsiyet") {
          ...
          if (!femaleRol || !maleRol) {
            // Eğer bu hatayı burada dönersen döngü durur; istersen continue ile atla
            return message.reply("❌ `@female` ve `@male` rolleri eksik!");
          }
        ...
        // hata olan üyeyi atlamak için devam et
        continue;
      }
    }

    return message.channel.send(
      `✅ Toplam **${count}** üye başarıyla kayıt edildi! (${
